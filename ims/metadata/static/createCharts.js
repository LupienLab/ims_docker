        ...
        var owner = new Chart(ctx2, {
          ...
          options: {
            ...
            onClick: function (evt) {
              var activePoints = owner.getElementsAtEventForMode(
                evt,
                "point",
                owner.options
              );
              ...
              var value =
                owner.data.datasets[firstPoint._datasetIndex].data[
                  firstPoint._index
                ];
      ...
      success: function (data) {
        ...
        var assay = new Chart(ctx3, {
          ...
          options: {
            ...
            onClick: function (evt) {
              var activePoints = assay.getElementsAtEventForMode(
                evt,
                "point",
                assay.options
              );
      ...
      success: function (data) {
        ...
        var disease = new Chart(ctx1, {
          ...
          options: {
            ...
            onClick: function (evt) {
              var activePoints = disease.getElementsAtEventForMode(
                evt,
                "point",
                disease.options
              );
              ...
              var value =
                disease.data.datasets[firstPoint._datasetIndex].data[
                  firstPoint._index
                ];
              window.location = "/browseProject/" + label + "/";
            },
      ...
      success: function (data) {
        ...
        var projectlabel = new Chart(ctx4, {
          ...
          options: {
            ...
            onClick: function (evt) {
              var activePoints = projectlabel.getElementsAtEventForMode(
                evt,
                "point",
                projectlabel.options
              );
              var firstPoint = activePoints[0];
              var label = projectlabel.data.labels[firstPoint._index];
              var value =
                projectlabel.data.datasets[firstPoint._datasetIndex].data[
                  firstPoint._index
                ];
              window.location = "/browseProject/" + label + "/";
            },
      ...
      success: function (data) {
        ...
        var experimenttags = new Chart(ctx5, {
          ...
          options: {
            ...
            onClick: function (evt) {
              var activePoints = experimenttags.getElementsAtEventForMode(
                evt,
                "point",
                experimenttags.options
              );
              var firstPoint = activePoints[0];
              var label = experimenttags.data.labels[firstPoint._index];
              var value =
                experimenttags.data.datasets[firstPoint._datasetIndex].data[
                  firstPoint._index
                ];
        ...
        return data.map((card) => {
          return card.map((value, index) => {
            if (index === 0) {
              ...
              return gridjs.html(
                `<a href='/browseExperimentGrid/${card[0]}/${value["assay"]}'>${value["value"]}</a>`
              );
            }
          });
        })
